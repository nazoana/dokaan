...
public class ClipboardKeyAdapter extends KeyAdapter {
	...
	private void copyToClipboard(boolean isCut) {
		...
		if (numRows != rowsSelected[rowsSelected.length - 1] - rowsSelected[0]
				+ 1
				|| numRows != rowsSelected.length
				|| numCols != colsSelected[colsSelected.length - 1]
						- colsSelected[0] + 1 || numCols != colsSelected.length) {

			Util.showError(null, "Invalid Copy Selection", "Invalid Selection");
			return;
		}
	...
	private void pasteFromClipboard() {
		...
		try {
			...
			Util.showError(null, "Invalid Paste Type", "Invalid Paste Type");
			return;
		}
